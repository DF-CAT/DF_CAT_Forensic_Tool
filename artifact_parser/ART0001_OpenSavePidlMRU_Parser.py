...
def OpenSavePidlMRU(userprofile,json_path, CSV, csv_path):
...
def Function_Start(pbarroot, pbar, data_dict,json_path, CSV, csv_path):
    data = {"version": "1.0.4", "ART0001": {"name": "OpenSavePidlMRU", "isEvent": False, "data": []}}
    ...
    try:
        ...
        if json_path is not None:
            with open(r"{}/ART0001_OpenSavePidlMRU.json".format(json_path), "w", encoding='utf-8') as json_file:
                json.dump(json_data, json_file, indent=4, ensure_ascii=False)
                json_file.close()

        if CSV != 0:
            with open(r"{}/ART0001_OpenSavePidlMRU.csv".format(csv_path), 'w', newline = '', encoding='ANSI') as output_file:
                f = csv.writer(output_file)

                # csv 파일에 header 추가
                f.writerow(["name", "extension", "open_time", "created_time", "modified_time", "size"])

                # write each row of a json file
                for datum in data["ART0001"]["data"]:
                    sleep(0.001)
                    pbar.step()
                    f.writerow([datum["name"], datum["extension"], datum["open_time"], datum["created_time"], datum["modified_time"], datum["size"]])
    except:
        pass
